...
/**
 * Build a `Map<legacySlug, price>` from the normalized server price list.
 *
...
export function buildPriceMap(serverPrices: any[]): Map<string, number> {
  ...
  for (const [key, entries] of byKey.entries()) {
    ...
    if (chosen && typeof chosen.price === 'number') priceMap.set(key, chosen.price);
  }
  return priceMap;
}
 ...
 */
export function mergeRates(prev: MaterialRate[], priceMap: Map<string, number>): MaterialRate[] {
